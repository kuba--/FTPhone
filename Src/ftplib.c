...
static int writeline(char *buf, int len, netbuf *nData)
{
  ...
  for (x=0; x < len; x++)
      ...
      if ((*ubp == '\n') && (lc != '\r'))
	{
	  if (nb == FTPLIB_BUFSIZ)
		...
		return x;
	      ...
	      if (w != FTPLIB_BUFSIZ)
		{
		  fprintf(stderr, "net_write(1) returned %d, errno = %d\n", w, errno);
		  return(-1);
		}
	      nb = 0;
	    }
      ...
      if (nb == FTPLIB_BUFSIZ)
	{
	  ...
	  if (w != FTPLIB_BUFSIZ)
	    {
	      fprintf(stderr, "net_write(2) returned %d, errno = %d\n", w, errno);
	      return(-1);
	    }
	  nb = 0;
	}
  ...
  if (nb)
	...
	return x;
      ...
      if (w != nb)
	{
	  fprintf(stderr, "net_write(3) returned %d, errno = %d\n", w, errno);
	  return(-1);
	}
    }
  return len;
		   ...
		   netbuf *nControl, int typ, int mode)
{
  ...
  if (typ == FTPLIB_FILE_WRITE)
	...
	if ((c = FtpWrite(dbuf, l, nData)) < l)
	  {
	    fprintf(stderr, "short write: passed %d, wrote %d\n", l, c);
	    rv = 0;
	    break;
	  }
    }
